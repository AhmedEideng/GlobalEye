...
export default function HomeMostRead({ articles }: { articles: NewsArticle[] }) {
  // Select 4 articles from the top (or random if no view order exists)
  const mostReadArticles = articles.slice(0, 4);
  
  if (!mostReadArticles?.length) return null;
  
  return (
    <section className="mb-10">
      <div className="section-header">
        <h2 className="section-title text-lg sm:text-xl font-bold">Most Read</h2>
