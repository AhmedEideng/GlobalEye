...
export async function generateMetadata({ searchParams }: { searchParams: { q?: string } }): Promise<Metadata> {
  ...
  const title = query ? `Search results for "${query}" | GlobalEye News` : 'Search | GlobalEye News';
  const description = query ? `Search results for "${query}" on GlobalEye News.` : 'Search for news, topics, or keywords on GlobalEye News.';
  const url = query ? `https://globaleye.news/search?q=${encodeURIComponent(query)}` : 'https://globaleye.news/search';
